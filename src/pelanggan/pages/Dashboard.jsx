...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    if (!error) {
      ...
      // Ambil data ID notifikasi yang sudah pernah dibaca dari localStorage
      ...
      const notifData = uniqueBookings.map((item) => ({
        ...
        // Cek apakah ID booking ini sudah terdaftar di localStorage sebagai 'sudah dibaca'
        isRead: readNotifIds.includes(item.id),
      }));
  ...
  // Fungsi saat tombol "Lihat Selengkapnya" ditekan
  const handleViewAllNotifications = () => {
    // Ambil semua ID dari notifikasi saat ini
    const allNotifIds = notifications.map((n) => n.id);
    
    // Simpan ke localStorage agar ketika balik ke dashboard, statusnya menetap sebagai 'sudah dibaca'
    localStorage.setItem("read_notifications", JSON.stringify(allNotifIds));
    
    // Pindah ke halaman notifikasi
    navigate("/pelanggan/notifikasi");
  };

  // Hitung jumlah badge merah hanya untuk notifikasi yang BELUM dibaca
            ...
            <div className="flex items-center gap-3 md:gap-4 self-start">
              ...
              <div className="relative">
                ...
                >
                  ...
                  {unreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 bg-rose-500 w-5 h-5 rounded-full text-[10px] font-bold flex justify-center items-center border-2 border-slate-900 text-white">
                      {unreadCount}
                    </span>
                  )}
                </button>
                    ...
                    <div className="max-h-64 overflow-y-auto divide-y divide-slate-100">
                      {notifications.length === 0 ? (
                        ...
                        notifications.slice(0, 3).map((item) => (
                          ...
                          >
                            {/* Gaya teks berubah tebal jika isRead false, dan normal jika true */}
                            <p className={`text-sm text-slate-700 ${item.isRead ? "font-normal opacity-70" : "font-bold"}`}>
                              {item.pesan}
                            </p>
